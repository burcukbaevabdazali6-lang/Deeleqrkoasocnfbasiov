...
local Window = Library:CreateWindow({
	Title = "Plutonium (Version 0.0.9)",
		...
		HudTransparency = 0,
		ImageTransparency = 0,
		Background = "139867571393999",
		BGTransparency = 100
	},

	CornerRadius = 17,
	CustomIcon = "122271988603244",
	ToggleKey = Enum.KeyCode.RightShift,
	Transparency = 0.25,
	ShowWatermark = { Enabled = true, Title = true, User = true, FPS = true, Duration = false, Ping = true },
	AutoSave = true,
	ConfigFolder = "Plutonium_Config"
})

-- Override default notification icon to 122271988603244 for all Library:Notify calls
local originalNotify = Library.Notify
Library.Notify = function(self, config, duration)
    local notifyConfig
    if type(config) == "table" then
        notifyConfig = config
    else
        notifyConfig = { Title = tostring(config or "Notification") }
        if type(duration) == "number" then
            notifyConfig.Duration = duration
        end
    end
    notifyConfig.ImageID = "rbxassetid://122271988603244"
    return originalNotify(self, notifyConfig)
end

local function makeSafeBlock(block)
    if type(block) ~= "table" then
        block = {}
    end
    local methods = {
        "CreateToggle", "CreateButton", "CreateSlider", "CreateInput", "CreateLabel",
        "CreateDropdown", "CreateColorPicker", "CreateKeybind", "CreateSection", "CreateBlock"
    }
    for _, name in ipairs(methods) do
        if type(block[name]) ~= "function" then
            block[name] = function() return nil end
        end
    end
    return block
end

local function safeBlock(tab, config)
    if type(tab) ~= "table" or type(tab.CreateBlock) ~= "function" then
        return makeSafeBlock(nil)
    end
    local ok, block = pcall(function()
        return tab:CreateBlock(config)
    end)
    if not ok or type(block) ~= "table" then
        return makeSafeBlock(nil)
    end
    return makeSafeBlock(block)
end

local function createTabSafe(name, twoColumn, icon)
    local ok, tab = pcall(function()
        return Window:CreateTab(name, twoColumn, icon)
    end)
    if not ok or type(tab) ~= "table" then
        Library.Notify(Library, {Title = "Plutonium", Content = "Failed to create tab: " .. name})
        return {
            CreateBlock = function() return makeSafeBlock(nil) end,
            CreateTab = function() return nil end
        }
    end

    if type(tab.CreateBlock) == "function" then
        local realCreateBlock = tab.CreateBlock
        tab.CreateBlock = function(self, cfg)
            local ok2, block = pcall(function()
                return realCreateBlock(self, cfg)
            end)
            if not ok2 or type(block) ~= "table" then
                return makeSafeBlock(nil)
            end
            return makeSafeBlock(block)
        end
...
local Tabs = {
    Home = createTabSafe("Home", true, "170940874"),
    Grab = createTabSafe("Grab", true, "17313313998"),
    Defense = createTabSafe("Defense", true, "16069233190"),
    Target = createTabSafe("Target", true, "107058246184363"),
    Blobmen = createTabSafe("Blobmen", true, "109717244852381"),
    Player = createTabSafe("Player", true, "124871982298256"),
    Visuals = createTabSafe("Visuals", true, "78134819718605"),
    Misc = createTabSafe("Misc", true, "114167292947807"),
    Server = createTabSafe("Server", true, "9692125126"),
    Keybinds = createTabSafe("Keybinds", true, "11710306232"),
}

local ServerLines, ServerPackets, ServerAll

local function ensureVisualAndServerTabs()
    if not Tabs.Visuals then
        Tabs.Visuals = Window:CreateTab("Visuals", true, "78134819718605")
    end
    if not Tabs.Server then
        Tabs.Server = Window:CreateTab("Server", true, "9692125126")
    end
end

local function ensureServerBlocks()
    ensureVisualAndServerTabs()
    if Tabs.Server and not ServerLines then
        ServerLines = Tabs.Server:CreateBlock({Name = "Lines", Side = "Left"})
    end
    if Tabs.Server and not ServerPackets then
        ServerPackets = Tabs.Server:CreateBlock({Name = "Packets", Side = "Left"})
    end
    if Tabs.Server and not ServerAll then
        ServerAll = Tabs.Server:CreateBlock({Name = "All", Side = "Right"})
    end
end
...
local accountCreatedText = "Approx. " .. tostring(accountAgeDays) .. " days ago"

local accountAgeDays = LocalPlayer.AccountAge or 0
local accountCreatedText = "Approx. " .. tostring(accountAgeDays) .. " days ago"

-- =========================================================================
-- PLUTONIUM ICON BLOCK (УВЕЛИЧЕННАЯ ВЕРСИЯ)
-- =========================================================================
local HomePlutoniumIcon = Tabs.Home:CreateBlock({Name = "Plutonium Icon", Side = "Left"})

-- Добавляем пустые Labels для увеличения высоты блока
HomePlutoniumIcon:CreateLabel({Text = " "})

-- Используем встроенный метод CreateImage из библиотеки!
HomePlutoniumIcon:CreateImage({
    Name = "Plutonium Logo",
    Flag = "PlutoniumIcon",
    ImageID = "122271988603244",  -- БЕЗ rbxassetid://
    Height = 260                  -- Увеличена высота до 280px
})

-- Добавляем ещё пустых Labels снизу
HomePlutoniumIcon:CreateLabel({Text = " "})

-- =========================================================================
-- ACCOUNT INFO
-- =========================================================================
local HomeInfo = Tabs.Home:CreateBlock({Name = "Account Info", Side = "Left"})
HomeInfo:CreateLabel({Text = "Username: " .. LocalPlayer.Name})
HomeInfo:CreateLabel({Text = "Display Name: " .. LocalPlayer.DisplayName})
HomeInfo:CreateLabel({Text = "User ID: " .. tostring(LocalPlayer.UserId)})
HomeInfo:CreateLabel({Text = "Account Age: " .. tostring(accountAgeDays) .. " days"})
HomeInfo:CreateLabel({Text = "Created: " .. accountCreatedText})

-- =========================================================================
-- CREATOR
-- =========================================================================
local HomeCredits = Tabs.Home:CreateBlock({Name = "Creator", Side = "Right"})
HomeCredits:CreateLabel({Text = "Creator: Hunter|115 IQ(Tested!)"})
HomeCredits:CreateLabel({Text = "Ai Creators:"})
HomeCredits:CreateLabel({Text = "Claude Sonnet 4.6"})
HomeCredits:CreateLabel({Text = "MAI Code FLASH-1"})
HomeCredits:CreateLabel({Text = "Raptor mini"})
HomeCredits:CreateLabel({Text = "Creator Icon: Chat GPT 5.5"})
HomeCredits:CreateLabel({Text = "Thanks for Buying Plutonium"})
HomeCredits:CreateLabel({Text = "Enjoy your experience!"})
HomeCredits:CreateLabel({Text = "Version: 0.0.9"})
HomeCredits:CreateLabel({Text = "Total: 12176 Lines of Code"})
...
function notify(title, content, duration)
	Library:Notify({ Title = title or "Notification", Content = content or "", Duration = duration or 5, ImageID = "rbxassetid://122271988603244" })
end
function sendHubLoadedMessage()
	local message = " Buyyed Version | Plutonium loaded. "
	local sent = false
	pcall(function()
...
-- Anti Grab [Home]
do
    local AntiGrabHomeEnabled = false
    local AntiGrabHomeConnection = nil
    local AntiGrabHomeCharacterConnection = nil
    local AntiGrabHomeTorsoConnection = nil
    local homeTeleportPosition = Vector3.new(-479.2, -7.4, -156.3)
    ...
    local function restoreTorsoGhost(char)
        if not char then return end
        local torsoPart = findTorsoPart(char)
        if torsoPart then
            pcall(function()
    ...
    local function applyAntiGrabHomeGhost()
        if not AntiGrabHomeEnabled then return end

        local char = Player.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local torsoPart = findTorsoPart(char)
        if not (char and hum and torsoPart and hum.Health > 0) then return end
        ...
        pcall(function()
            ...
            torsoPart.CFrame = CFrame.new(homeTeleportPosition)
        end)

        for _, part in ipairs(char:GetDescendants()) do
            if part:IsA("BasePart") and part ~= torsoPart then
                pcall(function()
                    part.Anchored = false
    ...
    local function setupAntiGrabHome()
        if AntiGrabHomeConnection then
            AntiGrabHomeConnection:Disconnect()
            AntiGrabHomeConnection = nil
        end
        if AntiGrabHomeCharacterConnection then
            AntiGrabHomeCharacterConnection:Disconnect()
            AntiGrabHomeCharacterConnection = nil
        end
        if AntiGrabHomeTorsoConnection then
            AntiGrabHomeTorsoConnection:Disconnect()
            AntiGrabHomeTorsoConnection = nil
        end

        AntiGrabHomeConnection = BeingHeld.Changed:Connect(function()
            if not AntiGrabHomeEnabled then return end
            if BeingHeld.Value then
                task.spawn(applyAntiGrabHomeGhost)
            end
        end)

        AntiGrabHomeCharacterConnection = Player.CharacterAdded:Connect(function(char)
            task.wait(0.5)
            if AntiGrabHomeEnabled and BeingHeld.Value then
                applyAntiGrabHomeGhost()
            end
        end)

        AntiGrabHomeTorsoConnection = Player.CharacterAdded:Connect(function(char)
    ...
    DefenseGroup:CreateToggle({
        ...
        Default = false,
        Callback = function(Value)
            SetToggleState("Anti Grab [Home]", Value)
            AntiGrabHomeEnabled = Value
            ...
            _G.AutoLeaveCon = game:GetService("ReplicatedStorage").GameCorrectionEvents.GameCorrectionsNotify.OnClientEvent:Connect(function(r)
                if r == "Flying" then
                    ...
                    if #warnTimestamps >= 3 then
                        game:GetService("Players").LocalPlayer:Kick("plutonium Safety: Disconnected to prevent ban.")
                    end
                end
            end)
        end
...
local activeAntiInput1, activeAntiInput2, activeAntiInput3, activeAntiInput4, activeAntiInput5 = nil, nil, nil, nil, nil

local function IsAntiInputItemUsedByOther(item, selfItem)
    for _, other in ipairs({activeAntiInput1, activeAntiInput2, activeAntiInput3, activeAntiInput4, activeAntiInput5}) do
        if other == item and other ~= selfItem then
            return true
        end
    end
    return false
...
local function AntiInputFindItem(name, currentItem)
    local plr = game.Players.LocalPlayer
    local toysFolder = workspace:FindFirstChild(plr.Name .. "SpawnedInToys")
    if toysFolder then
        for _, child in ipairs(toysFolder:GetChildren()) do
            if child.Name == name and child.Parent and not IsAntiInputItemUsedByOther(child, currentItem) then
                return child
            end
        end
    end
...
local function AntiInputSpawnItem(name, currentItem)
    local plr = game.Players.LocalPlayer
    local RS = game:GetService("ReplicatedStorage")
    ...
    pcall(function()
        if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            SpawnRemote:InvokeServer(name, plr.Character.HumanoidRootPart.CFrame * CFrame.new(0, -12, 0), Vector3.zero)
        else
            SpawnRemote:InvokeServer(name, CFrame.new(0, 0, 0), Vector3.zero)
        end
    end)

    local startTime = tick()
    while tick() - startTime < 2 and not spawnedItem do
        task.wait(0.05)
...
local function AntiInputGetItem(name, currentItem)
    local item = currentItem
    if not item or not item.Parent or item.Name ~= name then
        item = AntiInputFindItem(name, currentItem)
    end
    if not item or not item.Parent then
        item = AntiInputSpawnItem(name, currentItem)
    end
    return item
end
            ...
            task.spawn(function()
                ...
                while _G.InstantLagActive do
                    ...
                    if hrp then
                        ...
                        local item = AntiInputGetItem(name, AntiInputItem1)
                        AntiInputItem1 = item
                        activeAntiInput1 = item
...
local SelectedToy2 = ToyList["Burger"] or ToyList[DropdownValues[1]]
local SelectedToy3 = ToyList["Burger"] or ToyList[DropdownValues[1]]
local SelectedToy4 = ToyList["Burger"] or ToyList[DropdownValues[1]]
local SelectedToy5 = ToyList["Burger"] or ToyList[DropdownValues[1]]

local AntiInputItem1, AntiInputItem2, AntiInputItem3, AntiInputItem4, AntiInputItem5 = nil, nil, nil, nil, nil

DefenseExtra:CreateDropdown({
    Name = "Select Input Lag Toy 2",
    Flag = "InputLagToyDropdown2",
    Items = DropdownValues,
    Default = "Burger",
    Callback = function(Value)
        SelectedToy2 = ToyList[Value]
    end
})

DefenseExtra:CreateToggle({
    Name = "Anti-Input 2",
    Flag = "InstantInputLag2",
    Default = false,
    Callback = function(Value)
        SetToggleState("InstantInputLag2", Value)
        _G.InstantLagActive2 = Value
        ...
        if Value then
            task.spawn(function()
                ...
                while _G.InstantLagActive2 do
                    local char = plr.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    
                    if hrp then
                        local name = SelectedToy2
                        local item = AntiInputGetItem(name, AntiInputItem2)
                        AntiInputItem2 = item
                        activeAntiInput2 = item
...
DefenseExtra:CreateDropdown({
    Name = "Select Input Lag Toy 3",
    Flag = "InputLagToyDropdown3",
    Items = DropdownValues,
    Default = "Burger",
    Callback = function(Value)
        SelectedToy3 = ToyList[Value]
    end
})

DefenseExtra:CreateToggle({
    Name = "Anti-Input 3",
    Flag = "InstantInputLag3",
    Default = false,
    Callback = function(Value)
        SetToggleState("InstantInputLag3", Value)
        _G.InstantLagActive3 = Value
        ...
        if Value then
            task.spawn(function()
                ...
                while _G.InstantLagActive3 do
                    local char = plr.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    
                    if hrp then
                        local name = SelectedToy3
                        local item = AntiInputGetItem(name, AntiInputItem3)
                        AntiInputItem3 = item
                        activeAntiInput3 = item
...
DefenseExtra:CreateDropdown({
    Name = "Select Input Lag Toy 4",
    Flag = "InputLagToyDropdown4",
    Items = DropdownValues,
    Default = "Burger",
    Callback = function(Value)
        SelectedToy4 = ToyList[Value]
    end
})

DefenseExtra:CreateToggle({
    Name = "Anti-Input 4",
    Flag = "InstantInputLag4",
    Default = false,
    Callback = function(Value)
        SetToggleState("InstantInputLag4", Value)
        _G.InstantLagActive4 = Value
        ...
        if Value then
            task.spawn(function()
                ...
                while _G.InstantLagActive4 do
                    local char = plr.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    
                    if hrp then
                        local name = SelectedToy4
                        local item = AntiInputGetItem(name, AntiInputItem4)
                        AntiInputItem4 = item
                        activeAntiInput4 = item
...
DefenseExtra:CreateDropdown({
    Name = "Select Input Lag Toy 5",
    Flag = "InputLagToyDropdown5",
    Items = DropdownValues,
    Default = "Burger",
    Callback = function(Value)
        SelectedToy5 = ToyList[Value]
    end
})

DefenseExtra:CreateToggle({
    Name = "Anti-Input 5",
    Flag = "InstantInputLag5",
    Default = false,
    Callback = function(Value)
        SetToggleState("InstantInputLag5", Value)
        _G.InstantLagActive5 = Value
        ...
        if Value then
            task.spawn(function()
                ...
                while _G.InstantLagActive5 do
                    local char = plr.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    
                    if hrp then
                        local name = SelectedToy5
                        local item = AntiInputGetItem(name, AntiInputItem5)
                        AntiInputItem5 = item
                        activeAntiInput5 = item
...
local function getPlayerFromSelection(selection)
    ...
    if username then
        return game:GetService("Players"):FindFirstChild(username)
    end
    return nil
end
...
local BlobGroup = Tabs.Blobmen:CreateBlock({Name = "Blobman", Side = "Left", Icon = "109717244852381"})
local BlobmenESPGroup = Tabs.Blobmen:CreateBlock({Name = "Blobmen ESP", Side = "Right"})
...
-- =========================================================================
do
    local BlobESPEnabled = false
    local BlobESPBoxes = {}
    local ESPStyle = "Box" -- Box, Highlight, or Combined
    local ESPBrightness = 1
    local ESPGlowIntensity = 2
    local ESPConnection = nil

    local function RemoveAllBlobESP()
        for _, box in ipairs(BlobESPBoxes) do
    ...
    local function CreateESPBox(blob)
        if not blob or not blob:FindFirstChild("HumanoidRootPart") then return end
        ...
        textLabel.Text = "BLOBMAN"
        textLabel.BorderSizePixel = 2
        textLabel.BorderColor3 = Color3.fromRGB(0, 255, 0)
        textLabel.Parent = billboardGui

        if ESPStyle == "Highlight" or ESPStyle == "Combined" then
            pcall(function()
                local highlight = Instance.new("Highlight")
                highlight.FillColor = Color3.fromRGB(0, 255, 0)
                highlight.OutlineColor = Color3.fromRGB(0, 255, 0)
                highlight.FillTransparency = 0.5 / ESPBrightness
                highlight.OutlineTransparency = 0 / ESPGlowIntensity
                highlight.Parent = blob
            end)
        end

        table.insert(BlobESPBoxes, billboardGui)
    end

    local function StartBlobESP()
        BlobESPEnabled = true
        ...
        ESPConnection = game:GetService("RunService").Heartbeat:Connect(function()
            if not BlobESPEnabled then return end
            
            pcall(function()
                local workspace = game:GetService("Workspace")
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj.Name == "CreatureBlobman" and obj:FindFirstChild("HumanoidRootPart") then
                        local found = false
                        for _, box in ipairs(BlobESPBoxes) do
                            if box.Parent and box.Parent.Parent == obj then
                                found = true
                                break
                            end
                        end
    ...
    local function StopBlobESP()
        BlobESPEnabled = false
        if ESPConnection then
            ESPConnection:Disconnect()
            ESPConnection = nil
        end
    ...
    BlobmenESPGroup:CreateToggle({
        Name = "Enable Blobmen ESP",
        Flag = "BlobmenESP",
        Default = false,
        Callback = function(Value)
            if Value then
    ...
    BlobmenESPGroup:CreateDropdown({
        Name = "ESP Style",
        Flag = "ESPStyle",
        Items = {"Box", "Highlight", "Combined"},
        Default = "Box",
        Callback = function(Value)
            ESPStyle = Value
            RemoveAllBlobESP()
            if BlobESPEnabled then
                StartBlobESP()
            end
        end
    })

    BlobmenESPGroup:CreateSlider({
        Name = "ESP Brightness",
        Flag = "ESPBrightness",
        Min = 0.5,
        Max = 2,
    ...
    BlobmenESPGroup:CreateSlider({
        Name = "Glow Intensity",
        Flag = "ESPGlowIntensity",
        Min = 0.5,
        Max = 3,
        Default = 2,
        Increment = 0.1,
        Callback = function(Value)
            ESPGlowIntensity = Value
        end
    })
end

-- =========================================================================
-- BLOBMEN COLOR / TRANSPARENCY SYSTEM
-- =========================================================================
do
    local BlobmenVisualsGroup = Tabs.Blobmen:CreateBlock({Name = "Blobmen Visuals", Side = "Right"})
    local BlobColorEnabled = false
    local BlobColorConnection = nil
    local BlobColorName = "Green"
    local BlobTransparency = 0
    local BlobGlow = false

    local BlobColorMap = {
        Red = Color3.fromRGB(255, 45, 45),
        Blue = Color3.fromRGB(45, 110, 255),
        Green = Color3.fromRGB(60, 255, 90),
    ...
    local function ApplyBlobColor(blob)
        if not blob or not blob:IsA("Model") then return end

        local targetColor = BlobColorMap[BlobColorName] or Color3.fromRGB(255, 255, 255)

        for _, obj in ipairs(blob:GetDescendants()) do
            if obj:IsA("BasePart") then
                obj.Color = targetColor
                obj.Transparency = BlobTransparency
                if BlobGlow then
                    obj.Material = Enum.Material.Neon
                    obj.Color = obj.Color:Lerp(Color3.new(1, 1, 1), 0.18)
                else
    ...
    local function ApplyAllBlobColors()
        if not BlobColorEnabled then return end
        local workspace = game:GetService("Workspace")
        for _, obj in ipairs(workspace:GetDescendants()) do
    ...
    local function StartBlobColorSystem()
        BlobColorEnabled = true
        if BlobColorConnection then
            BlobColorConnection:Disconnect()
            BlobColorConnection = nil
        end

        ApplyAllBlobColors()

        local workspace = game:GetService("Workspace")
        BlobColorConnection = workspace.DescendantAdded:Connect(function(obj)
            if BlobColorEnabled and obj.Name == "CreatureBlobman" and obj:IsA("Model") then
                task.defer(function()
                    ApplyBlobColor(obj)
                end)
            end
    ...
    local function StopBlobColorSystem()
        BlobColorEnabled = false
        if BlobColorConnection then
            BlobColorConnection:Disconnect()
            BlobColorConnection = nil
        end
    ...
    BlobmenVisualsGroup:CreateDropdown({
        Name = "Blobmen Color",
        Flag = "BlobmenColor",
        Items = {"Red", "Blue", "Green", "Purple", "Yellow", "White", "Black"},
        Default = "Green",
        Callback = function(Value)
            BlobColorName = Value
            if BlobColorEnabled then
                ApplyAllBlobColors()
            end
        end
    })

    BlobmenVisualsGroup:CreateSlider({
        Name = "Blobmen Transparency",
        Flag = "BlobmenTransparency",
        Min = 0,
        Max = 1,
        Default = 0,
        Increment = 0.05,
        Callback = function(Value)
            BlobTransparency = Value
            if BlobColorEnabled then
                ApplyAllBlobColors()
            end
        end
    ...
    BlobmenVisualsGroup:CreateToggle({
        Name = "Blobmen Glow",
        Flag = "BlobmenGlow",
        Default = false,
        Callback = function(Value)
    ...
    BlobmenVisualsGroup:CreateToggle({
        Name = "Enable Blobmen Color",
        Flag = "EnableBlobmenColor",
        Default = false,
        Callback = function(Value)
            if Value then
                StartBlobColorSystem()
            else
                StopBlobColorSystem()
            end
        end
...
-- =========================================================================
-- BLOBMEN PLAYER SELECTOR (INDEPENDENT FROM TARGET)
-- =========================================================================
local selectedBlobmenPlayer = nil

-- Formats the list as "Display Name (@Username)"
local function getBlobmenPlayerList()
    local list = {}
    local Players = game:GetService("Players")
    local LocalPlayer = Players.LocalPlayer
    for _, plr in ipairs(Players:GetPlayers()) do
...
-- Extracts the username from the "Display Name (@Username)" string
local function getPlayerFromSelectionBlobmen(selection)
    if not selection or selection == "" then return nil end
    local username = selection:match("@(.-)%)")
    if username then
        return game:GetService("Players"):FindFirstChild(username)
...
-- 1. Blobmen Player Selector (LEFT SIDE)
local BlobmenPlayerDropdown = BlobGroup:CreateDropdown({
    Name = "Select Blobmen Target",
    List = getBlobmenPlayerList(),
    Default = nil,
    Callback = function(Value)
        selectedBlobmenPlayer = getPlayerFromSelectionBlobmen(Value)
        if selectedBlobmenPlayer then
            notify("Blobmen Target", "Selected: " .. selectedBlobmenPlayer.Name, 2)
        end
    end,
})
...
-- TARGET Dropdown Update Function
...
-- BLOBMEN Dropdown Update Function (INDEPENDENT)
local function updateBlobmenDropdown()
    local newList = getBlobmenPlayerList()
    
    if BlobmenPlayerDropdown then
        BlobmenPlayerDropdown:Refresh(newList, false)
    end
    
    -- Safety: If the blobmen target left the game, clear the variable
    if selectedBlobmenPlayer and not selectedBlobmenPlayer.Parent then
        selectedBlobmenPlayer = nil
    end
end

-- // Event Connections (FOR BOTH DROPDOWNS) \\ --

-- These listen for server changes to trigger the UI updates for BOTH independent systems
game:GetService("Players").PlayerAdded:Connect(function()
    updateTargetDropdown()
    updateBlobmenDropdown()
end)
...
Player.CharacterRemoving:Connect(function()
    -- Cleanup handled by PS.PlayerAdded/PlayerRemoving connections
end)
...
BlobGroup:CreateToggle({
    Name = "Plutonium (grab + blob)",
        Flag = "Plutonium (grab + blob)",
    Default = false,
    Callback = function(on)
        SetToggleState("Plutonium (grab + blob)", on)
        kickLoopEnabled = on
        loopKickDualActive = on
        
        if on then
            -- Initial Target Check
            if not selectedBlobmenPlayer then
                notify("Error", "Select target first", 3)
                -- Toggle Plutonium BlobKickToggle disabled
                return
            end
            ...
            task.spawn(function()
                ...
                local grabInterval = 0.10

                while kickLoopEnabled do
                    local target = selectedBlobmenPlayer
                    ...
                    if tRoot and tHum and tHum.Health > 0 then
                        ...
                        if not dragging then
                            ...
                            if tick() - grabStartTime > grabInterval then
                                dragging = true
                                grabStartTime = 0
                            end
                        else
...
-- =========================================================================
-- BLOBMEN (SPIN + GRAB) - Target Fixed in Center, You Spin Around
-- =========================================================================
do
    local SpinGrabActive = false
    local SpinSpeed = 5
    local SpinRadius = 8
    local SpinGrabConn = nil
    
    BlobGroup:CreateToggle({
        Name = "Blobmen (Spin + grab)",
        Flag = "BlobmenSpinGrab",
        Default = false,
        Callback = function(Value)
            SetToggleState("BlobmenSpinGrab", Value)
            SpinGrabActive = Value
            
            if SpinGrabConn then
                SpinGrabConn:Disconnect()
                SpinGrabConn = nil
            end
            
            if not Value then return end
            
            -- Проверка выбора цели
            if not selectedBlobmenPlayer then
                Library:Notify({ Title = "Error", Content = "Select target first!", Duration = 3 })
                SpinGrabActive = false
                return
            end
            ...
            task.spawn(function()
                ...
                -- Проверка что игрок сидит на блобмене
                if not myHum or not myHum.SeatPart then
                    Library:Notify({ Title = "Error", Content = "You must sit on Blobman first!", Duration = 3 })
                    SpinGrabActive = false
                    return
                end
                
                local blob = myHum.SeatPart.Parent
                if not blob or blob.Name ~= "CreatureBlobman" then
                    Library:Notify({ Title = "Error", Content = "Invalid Blobman!", Duration = 3 })
                    SpinGrabActive = false
                    return
                end
                ...
                if not blobHRP then
                    Library:Notify({ Title = "Error", Content = "Blobman HRP not found!", Duration = 3 })
                    SpinGrabActive = false
                    return
                end
                
                -- Получаем ремоуты блобмена
                local remoteFolder = blob:FindFirstChild("BlobmanSeatAndOwnerScript")
                local grab = remoteFolder and remoteFolder:FindFirstChild("CreatureGrab")
                local release = remoteFolder and remoteFolder:FindFirstChild("CreatureRelease")
                
                if not (grab and release) then
                    Library:Notify({ Title = "Error", Content = "Blobman remotes not found!", Duration = 3 })
                    SpinGrabActive = false
                    return
                end
                
                -- Получаем детекторы и weld'ы
                local L_Det = blob:FindFirstChild("LeftDetector")
                local R_Det = blob:FindFirstChild("RightDetector")
                local L_Weld = L_Det and (L_Det:FindFirstChild("LeftWeld") or L_Det:FindFirstChild("RigidConstraint"))
                local R_Weld = R_Det and (R_Det:FindFirstChild("RightWeld") or R_Det:FindFirstChild("RigidConstraint"))
                
                if not (L_Weld and R_Weld) then
                    Library:Notify({ Title = "Error", Content = "Blobman welds not found!", Duration = 3 })
                    SpinGrabActive = false
                    return
                end
                
                local angle = 0
                local centerPos = nil  -- Центр круга (позиция цели)
                local initialGrabDone = false
                
                Library:Notify({ Title = "Spin Grab", Content = "Spinning around target!", Duration = 3 })
                
                SpinGrabConn = RunService.Heartbeat:Connect(function()
                    if not SpinGrabActive then return end
                    
                    local target = selectedBlobmenPlayer
                    if not target or not target.Parent then
                        SpinGrabActive = false
                        Library:Notify({ Title = "Spin Grab", Content = "Target lost!", Duration = 2 })
                        return
                    end
                    ...
                    -- Установка центра при первом запуске
                    if not initialGrabDone then
                        centerPos = tRoot.Position
                        initialGrabDone = true
                    end
                    
                    -- ============================================
                    -- ФИКСАЦИЯ ЦЕЛИ В ЦЕНТРЕ КРУГА
                    -- ============================================
                    tRoot.CFrame = CFrame.new(centerPos)
                    tRoot.AssemblyLinearVelocity = Vector3.zero
                    tRoot.AssemblyAngularVelocity = Vector3.zero
                    
                    -- ============================================
                    -- GRAB/RELEASE SPAM (как в Plutonium grab + blob)
                    -- ============================================
                    pcall(function()
                        grab:FireServer(L_Det, tRoot, L_Weld)
                        grab:FireServer(R_Det, tRoot, R_Weld)
                    ...
                    -- Без задержки сразу release (максимальная скорость)
                    pcall(function()
                        release:FireServer(L_Weld)
                        release:FireServer(R_Weld)
                    end)
                    
                    -- ============================================
                    -- ВРАЩЕНИЕ ВОКРУГ ЦЕЛИ
                    -- ============================================
                    angle = angle + (SpinSpeed * 0.016) -- 0.016 ≈ 60 FPS
                    if angle >= 360 then angle = 0 end
                    
                    local radAngle = math.rad(angle)
                    local offsetX = math.cos(radAngle) * SpinRadius
                    local offsetZ = math.sin(radAngle) * SpinRadius
                    
                    -- Позиция блобмена на орбите (на уровне цели)
                    local orbitPos = Vector3.new(
                        centerPos.X + offsetX,
                        centerPos.Y,  -- Та же высота что и цель
                        centerPos.Z + offsetZ
                    )
                    
                    -- ============================================
                    -- БЛОБМЕН СМОТРИТ В ЦЕНТР (НА ЦЕЛЬ)
                    -- ============================================
                    local lookAtCenter = CFrame.lookAt(orbitPos, centerPos)
                    
                    -- Применяем позицию и направление
                    blobHRP.CFrame = lookAtCenter
                    blobHRP.AssemblyLinearVelocity = Vector3.zero
                    blobHRP.AssemblyAngularVelocity = Vector3.zero
                    
                    -- Обнуление скорости игрока (чтобы не слетел)
                    ...
                    -- Анти-рагдолл
                    pcall(function()
                        RS.CharacterEvents.RagdollRemote:FireServer(myRoot, 0)
                    end)
                    
                    -- Синхронизация позиции цели через сеть
                    pcall(function()
                        RS.GrabEvents.SetNetworkOwner:FireServer(tRoot, CFrame.new(centerPos))
                    end)
                end)
            end)
        end
...
BlobGroup:CreateToggle({
    Name = "Plutonium spam blob loop",
        Flag = "Plutonium spam blob loop", 
    Default = false,
    Callback = function(on)
        SetToggleState("Plutonium spam blob loop", on)
        loopKickDualActive = on
        if on then
            if not selectedBlobmenPlayer then
                notify("Error", "Select target first", 3)
...
BlobGroup:CreateToggle({
    Name = "Plutonium Kill Blob [Fast]",
        Flag = "Plutonium Kill Blob [Fast]", 
    Default = false,
    Callback = function(Value)
        SetToggleState("Plutonium Kill Blob [Fast]", Value)
        _G.LoopKillBlobActive = Value
        
        if Value then
            local tt = selectedBlobmenPlayer -- Your target variable
...
-- =========================================================================
-- NO CLIP (BLOB + ME) SYSTEM
-- =========================================================================
do
    local NoClipEnabled = false
    local NoClipConnection = nil
    local NoClipSpeed = 0.01
    ...
    local function StartNoClip()
        ...
        NoClipConnection = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            -- Make player transparent and no clip
            for _, part in ipairs(char:GetDescendants()) do
                if part:IsA("BasePart") then
                    part.CanCollide = false
                    if part.Transparency < 0.7 then
                        part.Transparency = part.Transparency + NoClipSpeed
                    end
                end
            end

            -- Find and make current blobman transparent and no clip
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum and hum.SeatPart then
                local seat = hum.SeatPart
                if seat.Parent and seat.Parent.Name == "CreatureBlobman" then
            ...
            if Value then
                ...
                spamGrabKickTask = task.spawn(function()
                    ...
                    while applymethod and RunService.RenderStepped:Wait() do
                        ...
                        if Sets.HRP and hum and HRP then
                            ...
                            if not kickbp or kickbp.Parent ~= Sets.HRP then
                                if Sets.HRP:FindFirstChild("Plutonium_BlobBP") then Sets.HRP.Plutonium_BlobBP:Destroy() end
                                kickbp = Instance.new("BodyPosition")
                                kickbp.Name = "Plutonium_BlobBP"
                            ...
                            if not kickbg or kickbg.Parent ~= Sets.HRP then
                                if Sets.HRP:FindFirstChild("Plutonium_BlobBG") then Sets.HRP.Plutonium_BlobBG:Destroy() end
                                kickbg = Instance.new("BodyGyro")
                                kickbg.Name = "Plutonium_BlobBG"
                    ...
                    if tt and tt.Character then
                        ...
                        if tHRP then
                            local oldBp = tHRP:FindFirstChild("Plutonium_BlobBP")
                            local oldBg = tHRP:FindFirstChild("Plutonium_BlobBG")
...
local LGK_OAT_grabKickFastEnabled = false
...
TargetGroup:CreateToggle({
    Name = "Grab Kick(Fast) baggy",
    Flag = "Grab Kick Fast",
    Default = false,
    Callback = function(on)
        SetToggleState("Grab Kick Fast", on)
        LGK_OAT_grabKickFastEnabled = on
        if not on then return end

        task.spawn(function()
            local target = selectedKickPlayer
            if not target or not target.Parent then
                LGK_OAT_grabKickFastEnabled = false
                Library:Notify({ Title = "Grab Kick(Fast)", Content = "No target selected.", Duration = 3 })
                return
            end
            ...
            while LGK_OAT_grabKickFastEnabled and RunService.Heartbeat:Wait() do
                target = selectedKickPlayer
                if not target or not target.Parent then break end
                ...
                if dist <= 30 then
                    ...
                    local align = tHRP:FindFirstChild("KickAlign")
                    if align and align.Attachment1 then
                        align.Attachment1.WorldPosition = myHead.Position + Vector3.new(0, 22, 0)
                    end
                    ...
                    if tick() - lastRemoteFire > 0.016 then
                        pcall(function()
                            ReplicatedStorage.GrabEvents.DestroyGrabLine:FireServer(tHRP)
                        end)
                        lastRemoteFire = tick()
...
-- Reorganized Grab UI: left and right blocks, placeholders for future features
_G.strength = _G.strength or 750
local GrabLock = Tabs.Grab:CreateBlock({Name = "Grab lock Selection", Side = "Left"})
local GrabAttacks = Tabs.Grab:CreateBlock({Name = "Grab Attacks", Side = "Right"})

GrabLock:CreateLabel({Text = "Grab lock Selection"})
GrabLock:CreateInput({
    Name = "Search Player",
    Flag = "Grab_SearchPlayer",
    Placeholder = "Type...",
    Callback = function(val) end
})
GrabLock:CreateDropdown({
    Name = "Select Lock Target - Target1",
    Flag = "Grab_SelectLockTarget",
    Items = {},
    Default = "",
    Callback = function(val) end
})
GrabLock:CreateLabel({Text = "lock Target - Mouse"})

GrabLock:CreateLabel({Text = "Grab Modifiers"})
GrabLock:CreateSlider({
    Name = "Throw Force",
    Flag = "Throw_Force",
    Default = _G.strength,
    Min = 1,
...
GrabLock:CreateToggle({
    Name = "Super Strength",
    Flag = "Super_Strength",
    Default = false,
    Callback = function(v)
        SetToggleState("Strength", v)
    end
})
GrabLock:CreateToggle({
    Name = "Massless Grab",
    ...
    Default = false,
    Callback = function(v)
        SetToggleState("Massless_Grab", v)
        _G.MassLessGrab = v
...
GrabLock:CreateToggle({
    Name = "Noclip Grab (Soon...)",
    Flag = "Noclip_Grab",
    Default = false,
    Callback = function(v) end
})
GrabLock:CreateToggle({
    Name = "Infinite Line Distance (Soon...)",
    Flag = "Infinite_Line",
    Default = false,
    Callback = function(v) end
})
GrabLock:CreateLabel({Text = "Jitter Mode [Pallet]"})
GrabLock:CreateToggle({
    Name = "Enable Jitter Mode (Soon...)",
    Flag = "Jitter_Mode",
    Default = false,
    Callback = function(v) end
})
GrabLock:CreateLabel({Text = "Magnet Key [Hold] (Soon...)"})

-- Right column: Attacks
GrabAttacks:CreateLabel({Text = "Grab Attacks"})
GrabAttacks:CreateToggle({
    Name = "Grab Attacks (Soon...)",
    Flag = "Grab_Attacks",
    Default = false,
    Callback = function(v) end
})
-- Kill grab logic already exists; toggle hooks into existing state
...
local function startKickGrab()
    ...
    kickGrabConnection = workspace.ChildAdded:Connect(function(v)
        if not kickGrabEnabled or not (v:IsA("Model") and v.Name == "GrabParts") then
            return
        end
        ...
        if grabPart and grabPart:FindFirstChild("WeldConstraint") then
            ...
            local targetChar = part1 and part1.Parent
            if targetChar and targetChar ~= Player.Character then
                pcall(function()
...
GrabAttacks:CreateToggle({
    Name = "Ragdoll Grab (Soon...)",
    Flag = "Ragdoll_Grab",
    Default = false,
    Callback = function(v) end
})
-- Figure Grab block moved here (keeps original logic unchanged)
do
    -- // Block Setup \ --
...
-- =========================================================================
-- PLAYER TAB - REORGANIZED STRUCTURE
-- =========================================================================
local PlayerMovement = Tabs.Player:CreateBlock({Name = "Movement", Side = "Left"})
local PlayerJump = Tabs.Player:CreateBlock({Name = "Jump", Side = "Left"})
local PlayerOther = Tabs.Player:CreateBlock({Name = "Other", Side = "Left"})
local PlayerColors = Tabs.Player:CreateBlock({Name = "Colors", Side = "Right"})
local PlayerVisuals = Tabs.Player:CreateBlock({Name = "Visuals", Side = "Right"})

-- =========================================================================
-- TELEPORT SECTION (Movement Block - Left)
-- =========================================================================
...
PlayerMovement:CreateToggle({
	...
	Default = false,
	Callback = function(Value)
		SetToggleState("Loop Teleport", Value)
	end
})

local tpLocations = {"Spawn", "Plot", "Shop"}
PlayerMovement:CreateDropdown({
	Name = "Teleport Location - Spawn - Spawn",
	Flag = "TP Location",
	Items = tpLocations,
	Default = "Spawn",
	Callback = function(Value)
...
-- =========================================================================
-- SPEED SECTION (Movement Block - Left)
-- =========================================================================
...
PlayerMovement:CreateToggle({
	...
	Default = false,
	Callback = function(Value)
		SetToggleState("Enable Speed", Value)
...
PlayerMovement:CreateKeybind({
	Name = "Speed Toggle Key",
	Flag = "Speed Toggle Key",
	Default = "V",
	Callback = function()
		PL_SpeedEnabled = not PL_SpeedEnabled
		if PL_SpeedEnabled and not PL_SpeedConn then
			PL_SpeedConn = RunService.RenderStepped:Connect(function()
				if not PL_SpeedEnabled then return end
				local char = Player.Character
				local hrp = char and char:FindFirstChild("HumanoidRootPart")
		...
		elseif not PL_SpeedEnabled and PL_SpeedConn then
			PL_SpeedConn:Disconnect()
			PL_SpeedConn = nil
		end
	end
...
PlayerMovement:CreateToggle({
	...
	Callback = function(Value)
		...
		if workspace.Map and workspace.Map:FindFirstChild("AlwaysHereTweenedObjects") then
			local oceanObj = workspace.Map.AlwaysHereTweenedObjects:FindFirstChild("Ocean")
			if oceanObj and oceanObj:FindFirstChild("Object") then
				local objectModel = oceanObj.Object:FindFirstChild("ObjectModel")
				if objectModel then
					for i, vv in pairs(objectModel:GetChildren()) do
...
local infJump = false
PlayerJump:CreateToggle({
	Name = "Infinite Jump",
	Flag = "Infinite Jump",
	Default = false,
	Callback = function(Value)
		SetToggleState("Infinite Jump", Value)
		infJump = Value
	end
})
...
-- =========================================================================
-- OTHER SECTION (Other Block - Left)
-- =========================================================================
local noclipEnabled = false
local noclipConn = nil

PlayerOther:CreateToggle({
	Name = "Noclip",
	Flag = "Noclip",
	Default = false,
	Callback = function(Value)
		SetToggleState("Noclip", Value)
		noclipEnabled = Value
		if Value then
...
-- =========================================================================
-- COLORS SECTION (Colors Block - Right)
-- =========================================================================
local pcldTransparency = 90
local pcldBoxColor = Color3.fromRGB(255, 255, 255)
local blobmanESPColor = Color3.fromRGB(255, 0, 0)

PlayerColors:CreateSlider({
	Name = "PCLD Transparency",
	Flag = "PCLD Transparency",
	Default = 90,
	Min = 0,
	Max = 100,
	Rounding = 0,
	Callback = function(Value)
		pcldTransparency = Value / 100
	end
})
...
PlayerColors:CreateColorPicker({
	Name = "Blobman ESP Color",
	Flag = "Blobman ESP Color",
	Default = Color3.fromRGB(255, 0, 0),
	Callback = function(Value)
		blobmanESPColor = Value
	end
})

-- =========================================================================
-- VISUALS SECTION (Visuals Block - Right)
-- =========================================================================
local pcldESPEnabled = false
local pcldESPBoxes = {}
local antiKickESPEnabled = false
local antiKickESPBoxes = {}
local blobmanESPEnabled = false
local blobmanESPBoxes = {}

-- PCLD ESP - сканирует парты PCLD
PlayerVisuals:CreateToggle({
	Name = "PCLD ESP",
	Flag = "PCLD ESP",
	Default = false,
	Callback = function(Value)
		SetToggleState("PCLD ESP", Value)
		pcldESPEnabled = Value
		if Value then
			-- Сканируем все части с именами PCLD
			for _, obj in ipairs(workspace:GetDescendants()) do
				if obj:IsA("BasePart") and (string.lower(obj.Name):find("pcld") or string.lower(obj.Name):find("partesp")) then
					if not pcldESPBoxes[obj] then
						local box = Instance.new("BoxHandleAdornment")
						box.Adornee = obj
						box.AlwaysOnTop = true
						box.ZIndex = 5
						box.Color3 = pcldBoxColor
						box.Transparency = pcldTransparency
						box.Size = obj.Size
						box.Parent = game.CoreGui
						pcldESPBoxes[obj] = box
						
						obj.AncestryChanged:Connect(function(_, parent)
							if not parent and pcldESPBoxes[obj] then
								pcldESPBoxes[obj]:Destroy()
								pcldESPBoxes[obj] = nil
							end
						end)
					end
				end
			...
			workspace.DescendantAdded:Connect(function(obj)
				if pcldESPEnabled and obj:IsA("BasePart") and (string.lower(obj.Name):find("pcld") or string.lower(obj.Name):find("partesp")) then
					if not pcldESPBoxes[obj] then
						local box = Instance.new("BoxHandleAdornment")
						box.Adornee = obj
						box.AlwaysOnTop = true
			...
			for _, box in pairs(pcldESPBoxes) do
...
-- Anti-Kick ESP - показывает AntiKick объекты
PlayerVisuals:CreateToggle({
	Name = "Anti-Kick ESP",
	Flag = "Anti-Kick ESP",
	Default = false,
	Callback = function(Value)
		SetToggleState("Anti-Kick ESP", Value)
		antiKickESPEnabled = Value
		if Value then
			for _, obj in ipairs(workspace:GetDescendants()) do
				if obj:IsA("BasePart") and (string.lower(obj.Name):find("antikick") or string.lower(obj.Name):find("kick")) then
					if not antiKickESPBoxes[obj] then
						local box = Instance.new("BoxHandleAdornment")
						box.Adornee = obj
						box.AlwaysOnTop = true
						box.ZIndex = 5
						...
						box.Size = obj.Size
						box.Parent = game.CoreGui
						antiKickESPBoxes[obj] = box
						
						obj.AncestryChanged:Connect(function(_, parent)
							if not parent and antiKickESPBoxes[obj] then
								antiKickESPBoxes[obj]:Destroy()
								antiKickESPBoxes[obj] = nil
							end
						end)
					end
				end
			...
			workspace.DescendantAdded:Connect(function(obj)
				if antiKickESPEnabled and obj:IsA("BasePart") and (string.lower(obj.Name):find("antikick") or string.lower(obj.Name):find("kick")) then
					if not antiKickESPBoxes[obj] then
						local box = Instance.new("BoxHandleAdornment")
						box.Adornee = obj
						box.AlwaysOnTop = true
			...
			for _, box in pairs(antiKickESPBoxes) do
				if box then box:Destroy() end
			end
			antiKickESPBoxes = {}
		end
...
-- ESP All Blobman - показывает всех blobmen
PlayerVisuals:CreateToggle({
	Name = "ESP All Blobman",
	Flag = "ESP All Blobman",
	Default = false,
	Callback = function(Value)
		SetToggleState("ESP All Blobman", Value)
		blobmanESPEnabled = Value
		if Value then
			-- Сканируем все CreatureBlobman на сервере
			for _, blob in ipairs(workspace:GetDescendants()) do
				if blob.Name == "CreatureBlobman" and blob:FindFirstChild("HumanoidRootPart") then
					local hrp = blob:FindFirstChild("HumanoidRootPart")
					if hrp and not blobmanESPBoxes[hrp] then
						local box = Instance.new("BoxHandleAdornment")
						box.Adornee = hrp
						box.AlwaysOnTop = true
						box.ZIndex = 5
						box.Color3 = blobmanESPColor
						box.Transparency = 0.3
						box.Size = hrp.Size
						box.Parent = game.CoreGui
						blobmanESPBoxes[hrp] = box
						
						hrp.AncestryChanged:Connect(function(_, parent)
							if not parent and blobmanESPBoxes[hrp] then
								blobmanESPBoxes[hrp]:Destroy()
								blobmanESPBoxes[hrp] = nil
							end
						end)
					end
				end
			end
			-- Следим за новыми бобменами
			workspace.DescendantAdded:Connect(function(obj)
				if blobmanESPEnabled and obj.Name == "CreatureBlobman" then
			...
			for _, box in pairs(blobmanESPBoxes) do
				if box then box:Destroy() end
			end
			blobmanESPBoxes = {}
		end
...
PlayerVisuals:CreateToggle({
	Name = "Blobman Name Tags",
	Flag = "Blobman Name Tags",
	Default = false,
	Callback = function(Value)
		SetToggleState("Blobman Name Tags", Value)
	end
})
...
PlayerVisuals:CreateToggle({
	...
	Default = false,
	Callback = function(Value)
		SetToggleState("Camera [Zoom]", Value)
		if Value then
...
PlayerVisuals:CreateToggle({
	...
	Default = false,
	Callback = function(Value)
		SetToggleState("Third Person", Value)
		if Value then
			enableThirdPerson()
		else
			disableThirdPerson()
...
PlayerVisuals:CreateToggle({
	...
	Default = false,
	Callback = function(Value)
		SetToggleState("FOV Changer", Value)
...
function playKickSound()
	...
	s.SoundId = "rbxassetid://86047092084443"
	s.Volume = 5
	s.PlayOnRemove = true
...
-- ===============================
-- NOTIFY (Plutonium Style)
-- ===============================
function notifyKick(displayName, username)
	local dispName = displayName and displayName ~= "" and displayName or (username or "Unknown")
	local officialName = username or "Unknown"
	Library:Notify({
		Title = "Plutonium",
		Content = "[" .. dispName .. " | " .. officialName .. "@] was Kicked out!!",
		Duration = 6
	})
end
...
-- =========================================================================
do
    local state = {active = false, conn = nil, target = nil}
    ...
    local function stopDogPose()
        ...
        if state.target and state.target.Parent then
            for _, part in ipairs(state.target:GetDescendants()) do
                if part:IsA("BasePart") then
                    part.AssemblyLinearVelocity = Vector3.zero
                    part.CanCollide = true
                end
    ...
    local function startDogPose()
        ...
        if not target or not target.Parent or not target.Parent:FindFirstChild("Humanoid") then
            return
        end

        state.target = target.Parent
        state.active = true

        state.conn = RunService.Heartbeat:Connect(function()
            if not state.active or not state.target or not state.target:FindFirstChild("Torso") then
                stopDogPose()
                return
            end
            ...
            local torso = state.target.Torso
            local head = state.target:FindFirstChild("Head")
            if head then
...
do
    ...
    Visuals.StartupSkyEnabled=true
    Visuals.StartupSkyAssetId = "rbxassetid://15359965253"
    local function applyStartupSky()
        if not Visuals.StartupSkyEnabled or not Visuals.StartupSkyAssetId or Visuals.StartupSkyAssetId == "" then return end
        for _, existingSky in ipairs(Lighting:GetChildren()) do
            if existingSky:IsA("Sky") then
                existingSky:Destroy()
            end
        end

        local ok, skyObj = pcall(function()
            return game:GetObjects(Visuals.StartupSkyAssetId)[1]
        end)
        if ok and skyObj and skyObj:IsA("Sky") then
            skyObj.Name = "Sky"
            skyObj.Parent = Lighting
            return
        end

        local fallbackSky = Instance.new("Sky")
        fallbackSky.SkyboxBk = Visuals.StartupSkyAssetId
        fallbackSky.SkyboxDn = Visuals.StartupSkyAssetId
        fallbackSky.SkyboxFt = Visuals.StartupSkyAssetId
        fallbackSky.SkyboxLf = Visuals.StartupSkyAssetId
        fallbackSky.SkyboxRt = Visuals.StartupSkyAssetId
        fallbackSky.SkyboxUp = Visuals.StartupSkyAssetId
        fallbackSky.Name = "Sky"
        fallbackSky.Parent = Lighting
    end
    applyStartupSky()
    ...
    FX_defs["Orbit Rings"] = function()
        ...
        for i = 1, count do
            ...
            ring.Name="Plutonium_FXPart"; ring.Size=Vector3.new(7,.18,.18)
    ...
    FX_defs["Bubble Shield"] = function()
        ...
        sphere.Name="PlutoniumFXPart"; sphere.Size=Vector3.new(8,8,8)
    ...
    FX_defs["Star Burst"] = function()
        ...
        for i=1,count do
            ...
            s.Name="PlutoniumFXPart"; s.Size=Vector3.new(.4,.4,.4)
    ...
    FX_defs["Ice Shards"] = function()
        ...
        for i=1,6 do
            local s=Instance.new("Part")
            s.Name="PlutoniumFXPart"; s.Size=Vector3.new(.3,1.4+math.random()*.8,.3)
    ...
    FX_defs["Shadow Clones"] = function()
        ...
        for i,off in ipairs(offsets) do
            ...
            clone.Name="PlutoniumShadowClone"
        ...
        task.spawn(function()
            while alive and Visuals.FX.ActiveEnabled do
                ...
                if hrp then
                    ...
                    m.Name="PlutoniumMeteor"; m.Size=Vector3.new(1,1,1)
...
-- =========================================================================
-- SERVER TAB → LINES (Low/Normal/Monster Lag System)
-- =========================================================================
do
    local LagModes = {
        Low = {
    ...
    local SelectedLagMode = "Normal"
    local LagEnabled = false
    local LagTask = nil

    ServerLines:CreateDropdown({
        Name = "Lag Mode",
        Flag = "ServerLagMode",
        Items = {"Low", "Normal", "Monster"},
        Default = "Normal",
        Callback = function(v)
            SelectedLagMode = v
        end
    ...
    ServerLines:CreateToggle({
        Name = "Enable Lag",
        Flag = "ServerEnableLag",
        Default = false,
        Callback = function(Value)
            LagEnabled = Value
            ...
            if Value then
                ...
                LagTask = task.spawn(function()
                    ...
                    if not CreateLine then
                        Library:Notify({ Title = "Error", Content = "CreateGrabLine not found", Duration = 3 })
                        return
                    end

                    local mode = LagModes[SelectedLagMode]
                    Library:Notify({ Title = mode.name, Content = "Enabled", Duration = 2 })
                    ...
                    while LagEnabled do
                        ...
                        if spawnLocation then
                            local randomX = math.random(mode.rangeX[1], mode.rangeX[2])
                            local randomZ = math.random(mode.rangeZ[1], mode.rangeZ[2])
                            pcall(function()
...
do
    ...
    ServerPackets:CreateSlider({
        Name     = "Packet Amount",
        Flag     = "ServerPacketAmount",
        Default  = 900,
        Min      = 50,
        Max      = 3000000,
        Rounding = 0,
    ...
    ServerPackets:CreateToggle({
        Name     = "Enable Packet Lag",
        Flag     = "ServerPacketLag",
        Default  = false,
        Callback = function(Value)
            SetToggleState("ServerPacketLag", Value)
            ...
            if Value then
                task.spawn(function()
                    ...
                    local GrabEvent = RS:WaitForChild("GrabEvents", 5)
                    if GrabEvent then
                        GrabEvent = GrabEvent:WaitForChild("ExtendGrabLine", 5)
                    end
...
do
    ...
    ServerAll:CreateButton({
        ...
        Callback = function()
            ...
            Library:Notify({ Title = "Kick All", Content = "Starting silent mass kick...", Duration = 3 })
            ...
            Library:Notify({ Title = "Success", Content = "Kick All execution complete!", Duration = 3 })
        end
    })
end
-- =========================================================================
-- RADAR SYSTEM (Beautiful Circular Radar with Adjustable Size)
-- =========================================================================
do
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    ...
    -- Настройки радара
    local RADAR_SIZE = 180        -- Размер радара (круга) - будет изменяться слайдером
    ...
    local function CreateRadar()
        ...
        RangeLabel.Text = "Range: " .. RADAR_RANGE .. " | Size: " .. RADAR_SIZE
    ...
    -- Пересоздание радара (при изменении размера)
    local function RecreateRadar()
        if not RadarEnabled then return end
        
        -- Сохраняем точки
        local savedDots = PlayerDots
        PlayerDots = {}
        ...
        for player, oldDot in pairs(savedDots) do
            oldDot:Destroy()
    ...
    -- Слайдер для изменения размера радара
    MiscGroup:CreateSlider({
        Name = "Radar Size",
        Flag = "RadarSize",
        Min = 100,
        ...
        Callback = function(v)
            RADAR_SIZE = v
            DOT_SIZE = math.max(6, math.floor(v / 25)) -- Автоматически масштабируем размер точек
            RecreateRadar()
        end
    })
    
    -- Слайдер для изменения дистанции
    MiscGroup:CreateSlider({
        Name = "Radar Range",
        Flag = "RadarRange",
        Min = 50,
        ...
        Callback = function(v)
            ...
            if RadarGui and RadarGui:FindFirstChild("RadarFrame") then
                ...
                if rangeLabel then
                    rangeLabel.Text = "Range: " .. v .. " | Size: " .. RADAR_SIZE
                end
            end
        end
    })
